...
const UPDATE_ALL_PAIRS_IN_NEVELLUSSWAP = 'UPDAUPDATE_ALL_PAIRS_IN_NEVELLUSSWAPTE_TOP_PAIRS'
const UPDATE_ALL_TOKENS_IN_NEVELLUSSWAP = 'UPDATE_ALL_TOKENS_IN_NEVELLUSSWAP'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_NEVELLUSSWAP: {
    ...
    case UPDATE_ALL_TOKENS_IN_NEVELLUSSWAP: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInNevellusSwap = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_NEVELLUSSWAP,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInNevellusSwap = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_NEVELLUSSWAP,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInNevellusSwap,
            updateAllTokensInNevellusSwap,
          },
        ],
        [
          state,
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.nevellusSwapFactories[0]
    ...
    oneDayData = oneDayResult.data.nevellusSwapFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.nevellusSwapFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.nevellusSwapFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.nevellusSwapFactories[0]
...
const getChartData = async (oldestDateToFetch, offsetData) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.nevellusSwapDayDatas)
      if (result.data.nevellusSwapDayDatas.length < 1000) {
        allFound = true
      }
    }
...
/**
 * Loop through every pair on NevellusSwap, used for search
 */
async function getAllPairsOnNevellusSwap() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on NevellusSwap, used for search
 */
async function getAllTokensOnNevellusSwap() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInNevellusSwap, updateAllTokensInNevellusSwap }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnNevellusSwap()
      updateAllPairsInNevellusSwap(allPairs)

      let allTokens = await getAllTokensOnNevellusSwap()
      updateAllTokensInNevellusSwap(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInNevellusSwap, updateAllTokensInNevellusSwap])
...
export function useAllPairsInNevellusSwap() {
...
export function useAllTokensInNevellusSwap() {
  const [state] = useGlobalDataContext()
