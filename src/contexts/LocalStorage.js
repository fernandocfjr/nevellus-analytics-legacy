...
const NEVELLUSSWAP = 'NEVELLUSSWAP'
...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(NEVELLUSSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(NEVELLUSSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
